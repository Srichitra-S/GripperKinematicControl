% Shows an animation of a 12 Dof robot hand (4 DoFs by finger). 
% thetaHand is a 12-length array containing the finger joints (in radians).

% It can receive an extra parameter, adjusting the viewpoint of the
% animation.

% If three extra parameters are given, it draws the manipulated object
% according to r, po, and Rpo (radius, CM position and rotation matrix).

% If four extra parameters are given, it draws the object and the fourth
% parameter is the viewpoint of the animation.

% It returns the current viewpoint.

function cview = Animate_Obj( thetaHand,r,po,Rpo,varargin )
...
%         cview = varargin{1};    % viewpoint
%     case 3
%         r = varargin{1};        % raio do objeto
%         po = varargin{2};       % posição do CM do objeto
%         Rpo = varargin{3};      % matriz de rotação do objeto
%     case 4
%         r = varargin{1};        % raio do objeto
%         po = varargin{2};       % posição do CM do objeto
%         Rpo = varargin{3};      % matriz de rotação do objeto
%         cview = varargin{4};    % viewpoint
% end

% Hand direct kinematics (angles in radians)
% [ p1 , ~ , p2 , ~ , p3 , ~ ] = Hand_Direct_Kinematics_Free ( thetaHand );

% Estima posição do objeto (cm)
% po = (mean([ p1 - GP(1,:)' , p2 - GP(2,:)' , p3 - GP(3,:)' ]')')./10;

% Parameters
l1 = 5.7; l2 = 3.8; l3 = 2.2;
l4a = 2.2; l4b = 2.2; l4c = 2.2;

% Posições das bases das falanges
...
% if (exist('r','var')) && (exist('po','var')) && (exist('Rpo','var'))
    % Contact points
    p_p1 = Rpo*([5 0 0]');
    p_p2 = Rpo*([0 5 0]');
    p_p3 = Rpo*([0 0 5]');
    ...
    % Draw arrows
    quiver3(po(2),-po(1),po(3),p_p1(2),p_p1(1),p_p1(3),'Color','g','LineWidth',2);
    quiver3(po(2),-po(1),po(3),p_p2(2),p_p2(1),p_p2(3),'Color','g','LineWidth',2);
    quiver3(po(2),-po(1),po(3),p_p3(2),p_p3(1),p_p3(3),'Color','g','LineWidth',2);
    
    % Draw ball
    mesh(y1*r+po(2),-x1*r-po(1),z1*r+po(3));
% end

% set(xx,'udata',po,'vdata',p_p1);
% set(yy,'udata',po,'vdata',p_p2);
% set(zz,'udata',po,'vdata',p_p3);
